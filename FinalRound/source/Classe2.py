class Pizza:
    ...
    def guillotine_solve(self):
        ...
        slice_score = {}
        slice_cut = {}
        for (i, j, k, l) in pizza_slices:
            slice_score[(i, j, k, l)] = 0
            slice_cut[(i, j, k, l)] = None
            if not self.is_slice_valid(i, j, k, l):
                continue
            if (k - i) * (l - j) <= self.max_size:
                slice_score[(i, j, k, l)] = (k - i) * (l - j)
            for vertical_cut in xrange(1, l - j):
                temp_score = slice_score[(i, j, k, j + vertical_cut)] + slice_score[(i, j + vertical_cut, k, l)]
            ...
            for horizontal_cut in xrange(1, k - i):
                temp_score = slice_score[(i, j, i + horizontal_cut, l)] + slice_score[(i + horizontal_cut, j, k, l)]
                if temp_score > slice_score[(i, j, k, l)]:
                    slice_score[(i, j, k, l)] = temp_score
    ...
    def is_slice_valid(self, i, j, k, l):
        # print i,j,k,l
        n_tomato = sum(self.grid[i + m][j:l].count("T") for m in xrange(k - i))
